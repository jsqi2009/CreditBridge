...
class DispatchClient(var mContext: Context, internal var mBus: AndroidBus) {

    fun enqueue(call: Call<JsonObject>, clasz: Class<*>, eventClass: Class<out BResponseEvent<*>>) {
    ...
    fun enqueue(o: Any, call: Call<JsonObject>, clasz: Class<*>, eventClass: Class<out BResponseEvent<*>>) {
        val dispatchCallbackBusAdapter = DispatchCallbackBusAdapter(mBus, eventClass, o)
        val callback = InternalCallback(clasz, dispatchCallbackBusAdapter)
        call.enqueue(callback)
    }

    fun enqueue(call: Call<JsonObject>, clasz: Class<*>, eventClass: Class<out BResponseEvent<*>>, flag: String) {
        val dispatchCallbackBusAdapter = DispatchCallbackBusAdapter(mBus, eventClass)
        val callback = InternalCallback(clasz, dispatchCallbackBusAdapter, flag)
        call.enqueue(callback)
    }

    fun enqueue(call: Call<JsonObject>, clasz: Class<*>, eventClass: Class<out BResponseEvent<*>>, flag: String, position: Int) {
        val dispatchCallbackBusAdapter = DispatchCallbackBusAdapter(mBus, eventClass)
        val callback = InternalCallback(clasz, dispatchCallbackBusAdapter, flag, position)
        call.enqueue(callback)
    }
    ...
    private inner class InternalCallback : Callback<JsonObject> {
        ...
        private fun getGson(jsonObject: JsonObject?): Gson {
            var gson: Gson? = null
            if (jsonObject != null) {
                val errNumJsonObject = jsonObject.get("success")
                val errNum = errNumJsonObject.asBoolean
                if (errNum) {
                    gson = Gson()
                } else {
